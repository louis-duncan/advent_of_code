...
data = """fdaebc egc gc bgefc ecbagd becfa fagc afgbec gdcbfae bgdef | cgfa gcebad ebcadfg cefgbad
...
all_mappings = list(["".join(_m) for _m in itertools.permutations("abcdefg")])
...
def does_string_overlap(string1: str, string2: str) -> bool:
    for c in string1:
        if c in string2:
            return True
    return False
...
def does_mapping_overlap(subject_mapping: str, filter_mapping: Dict[str, str]) -> bool:
    for i, c in enumerate("abcdefg"):
        if subject_mapping[i] not in filter_mapping[c]:
            return False
    return True


def pattern_to_bools(pattern: str, mapping: str = "abcdefg") -> List[bool]:
    return [c in pattern for c in mapping]


def bools_to_pattern(bools: List[bool], mapping: str = "abcdefg") -> str:
    return "".join(sorted([c for b, c in zip(bools, mapping) if b]))


def bools_to_digit(bools: List[bool], mapping: str = "abcdefg") -> int:
    global true_mapping
    pattern = "".join(sorted(bools_to_pattern(bools, mapping)))
    for d, v in true_mapping.items():
        if pattern == v:
            return d
    else:
        raise ValueError(f"Could not convert {pattern=} to digit with {mapping=}")


def pattern_to_digit(pattern: str, mapping: str = "abcdefg") -> int:
    return bools_to_digit(pattern_to_bools(pattern, mapping), "abcdefg")


def mapping_is_valid(subject_mapping: str, samples: List[str]) -> bool:
    global true_mapping
    for s in samples:
        converted = bools_to_pattern(pattern_to_bools(s, subject_mapping), "abcdefg")
        if converted not in true_mapping.values():
            return False
    return True


def process_line(line: str) -> int:
    global all_mappings, true_mapping
    examples_str, final_str = line.split(" | ")
    examples: List[str] = examples_str.split()
    examples.sort(key=len)
    output_digits: List[str] = final_str.split()
    ...
    for example in examples:
        if len(example) == 2:  # Is a 1.
            for c in "abcdefg":
                if c in true_mapping[1]:
                    new_mapping[c] = "".join(filter(lambda x: x in example, new_mapping[c]))
                else:
                    new_mapping[c] = "".join(filter(lambda x: x not in example, new_mapping[c]))

        elif len(example) == 3:  # Is a 7.
            for c in "abcdefg":
                if c in true_mapping[7]:
                    new_mapping[c] = "".join(filter(lambda x: x in example, new_mapping[c]))
                else:
                    new_mapping[c] = "".join(filter(lambda x: x not in example, new_mapping[c]))

        elif len(example) == 4:  # Is a 4.
            for c in "abcdefg":
                if c in true_mapping[4]:
                    new_mapping[c] = "".join(filter(lambda x: x in example, new_mapping[c]))
                else:
                    new_mapping[c] = "".join(filter(lambda x: x not in example, new_mapping[c]))
    ...
    filtered_mappings = filter(lambda m: does_mapping_overlap(m, new_mapping), all_mappings)
    for fm in filtered_mappings:
        if mapping_is_valid(fm, examples):
            break
    else:
        raise ValueError("Failed to find mapping.")

    num_str = "".join([str(pattern_to_digit(o, fm)) for o in output_digits])
    return int(num_str)
...
def simple(line: str):
    ...
    examples: List[str] = examples_str.split()
    examples.sort(key=len)
    output: List[str] = final_str.split()
...
def star_one():
    ...
    for line in data.split("\n"):
        new_counts = simple(line)
        for k in new_counts:
            counts[k] += new_counts[k]
    print(counts)
...
def star_two():
    global data
    values = [process_line(line) for line in data.split("\n")]
    print(sum(values))


def main():
    # print(process_line("acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf"))
    star_two()
